...
const MASTER_ORACLE = '0x1887118E49e0F4A78Bd71B792a49dE03504A764D'
const MASTER_ORACLE_ADMIN = '0x5ea4a9a7592683bf0bc187d6da706c6c4770976f'
...
describe('DeltaNeutralStableVolatilePairUpgradeable', function () {
    ...
    before(async function () {
        ...
        const AhOracleFactory = await ethers.getContractFactory('AutoHedgeOracle')
        const UniLpOracleFactory = await ethers.getContractFactory('UniswapLpTokenPriceOracle')
        ...
        ahOracle = await AhOracleFactory.deploy(weth.address)
        uniLpOracle = await UniLpOracleFactory.deploy(weth.address)
        ...
        // impersonate master oracle admin in order to be able to add our own oracle
        await hre.network.provider.request({
            method: "hardhat_impersonateAccount",
            params: [MASTER_ORACLE_ADMIN],
        });
        ...
        // stop impersonating and reset back to original accounts
        await hre.network.provider.request({
    ...
    it.skip('Should deposit twice', async function () {
        // I'm aware this is a super noob move - just duct taping to save time
        testSnapshotId = await revSnapshot(testSnapshotId)
    ...
    it('Should check oracle', async function () {
        ...
        const amount = await pair.connect(alice).callStatic.withdraw(
            aliceLiquidityWithdraw,
            [
                amountStableMin,
                amountVolMin,
