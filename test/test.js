...
const {getEthPrice, getAddresses, equalTol, revSnapshot, noDeadline} = require('../scripts/utils')
...
describe('DeltaNeutralStableVolatilePairUpgradeable', function () {
    ...
    before(async function () {
        ...
        const DeltaNeutralStableVolatilePairUpgradeable = await ethers.getContractFactory('DeltaNeutralStableVolatilePairUpgradeable')
        ...
        pair = await DeltaNeutralStableVolatilePairUpgradeable.attach(receipt.events[receipt.events.length-1].args.pair)
    ...
    it('Should rebalance, borrow more ETH, no fee', async function () {
        ...
        await expect(pair.rebalance(0)).to.be.revertedWith(REV_MSG_WITHIN_RANGE);
        ...
        const estStableFromVol = (await uniV2Router.getAmountsOut(amountVolOwned.sub(amountVolDebt), [weth.address, dai.address]))[1]
        
        await pair.rebalance(0)
        ...
        expect(await dai.balanceOf(pair.address)).to.equal(0)
        expect(await dai.balanceOf(owner.address)).to.equal(daiBalanceBefore.sub(amountStable).sub(amountsStableToVol[0]))
        equalTol(await cStable.callStatic.balanceOfUnderlying(pair.address), amountStableSwappedIntoEstimated.add(estStableFromVol))
    ...
    it('Should rebalance, borrow more ETH with a fee, pair does not have enough stables to cover a fee', async function () {
        // I'm aware this is a super noob move - just duct taping to save time
        testSnapshotId = await revSnapshot(testSnapshotId)
        ...
        // to estimate the amount of stables to swap for the fee, need to take the snapshot and execute the swap
        testSnapshotId3 = await network.provider.request({
            method: 'evm_snapshot'
        })

        await weth.approve(uniV2Router.address, amountStableInit)
        await uniV2Router.swapExactTokensForTokens(amountVolOwned.sub(amountVolDebt), 1, [weth.address, dai.address], owner.address, TEN_18)
        // Set the rebalance fee(ETH), set 30% of the borrow amount to avoid the case there aren't enough stables to cover the execution fee
        const feeAmount = (amountVolOwned.sub(amountVolDebt)).mul(30).div(100);
        const estStableFromVolFee = (await uniV2Router.getAmountsIn(feeAmount, [dai.address, weth.address]))[0]

        await network.provider.request({
            method: 'evm_revert',
            params: [testSnapshotId3]
        })

        await pair.rebalance(feeAmount);
        ...
        expect(await dai.balanceOf(pair.address)).to.equal(0)
        expect(await dai.balanceOf(owner.address)).to.equal(daiBalanceBefore.sub(amountStable).sub(amountsStableToVol[0]))
        equalTol(await cStable.callStatic.balanceOfUnderlying(pair.address), amountStableSwappedIntoEstimated.add(estStableFromVol).sub(estStableFromVolFee))
    ...
    it('Should rebalance, borrow more ETH with a fee, pair has enough stables to cover a fee', async function () {
        // I'm aware this is a super noob move - just duct taping to save time
        testSnapshotId = await revSnapshot(testSnapshotId)
        ...
        // supply enough ETH to cover a fee
        const feeAmount = (amountVolOwned.sub(amountVolDebt)).mul(30).div(100);
        await owner.sendTransaction({to: pair.address, value: amountVolOwned.sub(amountVolDebt)});
    ...
    it('Should rebalance, repay ETH, with a fee', async function () {
        // I'm aware this is a super noob move - just duct taping to save time
        testSnapshotId = await revSnapshot(testSnapshotId)
        ...
        // Set the rebalance fee(ETH), set 30% of the repay amount
