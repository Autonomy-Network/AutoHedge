...
describe("DeltaNeutralStableVolatilePairUpgradeable", () => {
  ...
  describe("rebalance()", () => {
    ...
    it("Should rebalance, borrow more ETH, no fee, no triggered by people adding liquidity", async () => {
      const amountStableInit = parseEther(String(1.1 * ethPrice * 2)) // fuse min borrow amount is 1 ETH, and half is kept as DAI
      const amountVolZapMin = parseEther("1")
      const amountStableMin = 0
      const amountVolMin = 0
      ...
      // Should revert when trying to rebalance when it's not needed
      await expect(pair.connect(alice).rebalance(false)).to.be.revertedWith(
        REV_MSG_WITHIN_RANGE
      )
    ...
    it("Should rebalance, repay ETH, no fee, use weth, no triggered by people adding liquidity", async () => {
      const amountStableInit = parseEther(String(1.1 * ethPrice * 2)) // fuse min borrow amount is 1 ETH, and half is kept as DAI
      const amountVolZapMin = parseEther("1")
      const amountStableMin = 0
      const amountVolMin = 0
    ...
    it("Should rebalance, neither repay nor borrow, no fee, use weth, no triggered by people adding liquidity", async () => {
      const amountStableInit = parseEther(String(1.1 * ethPrice * 2)) // fuse min borrow amount is 1 ETH, and half is kept as DAI
      const amountVolZapMin = parseEther("1")
      const amountStableMin = 0
      const amountVolMin = 0
