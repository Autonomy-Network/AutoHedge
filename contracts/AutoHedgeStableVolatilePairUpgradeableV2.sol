...
*/
contract AutoHedgeStableVolatilePairUpgradeableV2 is IAutoHedgeStableVolatilePairUpgradeableV2, IFlashloanWrapper, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UniswapV2ERC20Upgradeable {
    ...
    function initialize(
        ...
        require(results[0] == 0 && results[1] == 0, "DNPair: unable to enter markets");
    ...
    struct FinishDeposit {
        ...
        uint amountVolLiq;
        uint amountStableLiqMin;
        uint amountVolLiqMin;
    ...
    function onFlw(
        uint fee,
        bytes calldata data
    ) external override nonReentrant returns (uint, uint, uint) {
        Tokens memory _tokens = tokens; // Gas savings
        
        // Can only trust calls that originated from the place we intentionally route them through
        require(msg.sender == factory.flw(), "AHPair: not flw");
        (IFlashloanWrapper.FinishRoute fr) = abi.decode(data[:64], (IFlashloanWrapper.FinishRoute));

        // Can only trust calls that originated from this contract
        require(fr.flwCaller == address(this), "AHPair: flw caller not this");

        if (fr.flt == IFlashloanWrapper.FlashLoanType.Deposit) {
            return _finishDeposit(_tokens, fee, data);
        } else {
    ...
    function _finishDeposit(
        ...
        uint fee,
        bytes calldata data
    ) private returns (uint amountStable, uint amountVol, uint amountUniLp) {
        (FinishDeposit fd) = abi.decode(data[64:], (FinishDeposit));

        // Get stables from the user
        _tokens.stable.safeTransferFrom(fd.depositor, fd.amountStableLiq + fd.amountStableFee);
        ...
        uint[] memory amountsStableToVol = uniV2Router.swapTokensForExactTokens(
            fee,
            fd.amountStableInit,
            fd.pathStableToVol,
            address(this),
        ...
        if (fd.amountStableInit - amountsStableToVol[0] > amountStable) {
            _tokens.stable.safeTransfer(fd.depositor, fd.amountStableInit - amountsStableToVol[0] - amountStable);
        }
        ...
        // Repay the flashloan
        _tokens.vol.safeTransfer(msg.sender, fd.amountVolLiq + fee);

        emit Deposited(fd.depositor, amountStable, amountVol, amountUniLp, amountsStableToVol[0], liquidityForUser);
    }
