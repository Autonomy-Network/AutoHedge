...
import "../../interfaces/autonomy/IStakeManager.sol";
import "../../interfaces/autonomy/IOracle.sol";
...
contract StakeManager is
    ...
    IERC777Recipient
{
    uint256 public constant STAN_STAKE = 10000 * _E_18;
    uint256 public constant BLOCKS_IN_EPOCH = 100;
    ...
    mapping(address => uint256) private _stakerToStakedAmount;
    ...
    function setAUTO(IERC777 AUTO) external override {
    ...
    function getStakesLength() external view override returns (uint256) {
        ...
        returns (address[] memory)
    {
        ...
        uint256 sliceIdx = 0;
        for (uint256 stakeIdx = startIdx; stakeIdx < endIdx; stakeIdx++) {
        ...
        )
    {
        epoch = getCurEpoch();
        // So that the storage is only loaded once
        uint256 stakesLen = _stakes.length;
    ...
    function isUpdatedExec(address addr)
        external
        override
        nonReentrant
        noFish
    ...
    function stake(uint256 numStakes)
        external
        override
        nzUint(numStakes)
        nonReentrant
        ...
        noFish
    {
        uint256 amount = numStakes * STAN_STAKE;
        _stakerToStakedAmount[msg.sender] += amount;
        // So that the storage is only loaded once
        IERC777 AUTO = _AUTO;

        // Deposit the coins
        uint256 balBefore = AUTO.balanceOf(address(this));
        AUTO.operatorSend(
            msg.sender,
            address(this),
            amount,
        ...
        require(
            AUTO.balanceOf(address(this)) - balBefore == amount,
            "SM: transfer bal check failed"
        );

        for (uint256 i; i < numStakes; i++) {
    ...
    function unstake(uint256[] calldata idxs)
        external
        override
        nzUintArr(idxs)
        nonReentrant
        ...
        noFish
    {
        uint256 amount = idxs.length * STAN_STAKE;
        require(
            amount <= _stakerToStakedAmount[msg.sender],
            "SM: not enough stake, peasant"
        );
    ...
    modifier noFish() {
        ...
        require(
            _AUTO.balanceOf(address(this)) >= _totalStaked,
            "SM: something fishy here"
        );
    }
    ...
    function tokensReceived(
        ...
        require(
            keccak256(_operatorData) == keccak256(_stakingIndicator),
            "SM: sending by mistake"
        );
    }
}
