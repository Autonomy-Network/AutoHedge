 ...
 */
contract DeltaNeutralStableVolatilePairUpgradeable is
    IDeltaNeutralStableVolatilePairUpgradeable,
    Initializable,
    OwnableUpgradeable,
    ...
    UniswapV2ERC20Upgradeable
{
    ...
    function initialize(
        ...
        require(
            ...
            "DNPair: unable to enter markets"
        );
        ...
        )
    {
        ...
        require(
            uniArgs.pathVolToStable[0] == address(_tokens.vol) &&
                uniArgs.pathVolToStable[uniArgs.pathVolToStable.length - 1] ==
                address(_tokens.stable) &&
                uniArgs.pathStableToVol[0] == address(_tokens.stable) &&
                uniArgs.pathStableToVol[uniArgs.pathStableToVol.length - 1] ==
                address(_tokens.vol),
        ...
        _tokens.stable.safeTransferFrom(
            msg.sender,
            address(this),
            amountStableInit
        );
        uint256[] memory amountsStableToVol = uniV2Router
        ...
        if (amountStableInit > amountsStableToVol[0] + amountStable) {
            _tokens.stable.safeTransfer(
                msg.sender,
                amountStableInit - amountsStableToVol[0] - amountStable
            );
        }
        ...
        uint256 currentUniLpBal = _tokens.cUniLp.balanceOfUnderlying(
            address(this)
        );
        ...
        uint256 code = _tokens.cUniLp.mint(amountUniLp);
        require(
            code == 0,
            string(
                abi.encodePacked(
        ...
        require(
            code == 0,
            string(
                abi.encodePacked("DNPair: fuse borrow ", Strings.toString(code))
            )
        );

        // Swap the volatile token for the stable token
        uint256[] memory amountsVolToStable = uniV2Router
            .swapExactTokensForTokens(
                amountVol,
        ...
        uint256 lendIncrease = (_tokens.cStable.balanceOfUnderlying(
            address(this)
        ) * increaseFactor) / BASE_FACTOR;
        ...
        if (
            lendIncrease == 0 ||
            lendIncrease == amountsVolToStable[amountsVolToStable.length - 1]
        ) {
            code = _tokens.cStable.mint(
                amountsVolToStable[amountsVolToStable.length - 1]
            );
        } else {
        ...
        returns (uint256 amountStableToUser)
    {
        ...
        uint256 amountStableFromLending = (_tokens.cStable.balanceOfUnderlying(
            address(this)
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "DNPair: fuse stable redeem ",
                    Strings.toString(code)
                )
            )
        );

        uint256 amountVolFromStable = uniV2Router.swapExactTokensForTokens(
        ...
        uint256 amountVolToRepay = (_tokens.cVol.borrowBalanceCurrent(
            address(this)
        ) * liquidity) / _totalSupply;
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "DNPair: fuse vol repay ",
                    Strings.toString(code)
                )
            )
        );
        uint256 amountUniLp = (_tokens.cUniLp.balanceOfUnderlying(
            address(this)
        ) * liquidity) / _totalSupply;
        ...
        if (amountVolToRepay <= amountVolFromStable) {
            ...
            require(
                ...
                string(
                    abi.encodePacked(
                        "DNPair: fuse LP redeem 1 ",
                        Strings.toString(code)
                    )
                )
            );

            (, uint256 amountVolFromDex) = uniV2Router.removeLiquidity(
                address(_tokens.stable),
                address(_tokens.vol),
                amountUniLp,
                uniArgs.amountStableMin,
            ...
            uint256 amountUniLpPaidFirst = (amountUniLp * amountVolFromStable) /
                amountVolToRepay;
            ...
            require(
                amountVolFromDex > amountVolToRepay - amountVolFromStable,
                "DNPair: vol cant cover defecit"
            );
            code = _tokens.cVol.repayBorrow(
                amountVolToRepay - amountVolFromStable
            );
            require(
                code == 0,
                string(
                    abi.encodePacked(
                        "DNPair: fuse vol repay 2 ",
                        Strings.toString(code)
                    )
                )
            );
            ...
            code = _tokens.cUniLp.redeemUnderlying(
            ...
            require(
                ...
                string(
                    abi.encodePacked(
                        "DNPair: fuse LP redeem 2 ",
                        Strings.toString(code)
                    )
                )
            );

            (, uint256 amountVolFromDex2) = uniV2Router.removeLiquidity(
                address(_tokens.stable),
                address(_tokens.vol),
                amountUniLp - amountUniLpPaidFirst,
                (uniArgs.amountStableMin *
                    (amountUniLp - amountUniLpPaidFirst)) / amountUniLp,
                (uniArgs.amountVolMin * (amountUniLp - amountUniLpPaidFirst)) /
                    amountUniLp,
                address(this),
                uniArgs.deadline
            );
     ...
     */
    function rebalanceAuto(address user, uint256 feeAmount)
        public
        override
        nonReentrant
    {
    ...
    function _rebalance(uint256 feeAmount, bool passIfInBounds) private {
        Tokens memory _tokens = tokens; // Gas savings
        ...
        if (volPos.bps >= mb.max) {
            ...
            if (_tokens.vol == weth) {
                // Get enough WETH
                uint256 amountVolNeeded = !payFeeFromBal
                    ? amountVolToRepay + feeAmount
                    : amountVolToRepay;
                amountStableToRedeem = uniV2Router.getAmountsIn(
                    amountVolNeeded,
                    pathStableToVol
                )[0];
                ...
                uint256 amountStableForDebt = uniV2Router.getAmountsIn(
                    amountVolToRepay,
                    pathStableToVol
                )[0];
                ...
                require(
                    ...
                    string(
                        abi.encodePacked(
                            "DNPair: fuse repay borrow non-WETH ",
                            Strings.toString(code)
                        )
                    )
                );
                ...
                if (feeAmount > 0 && !payFeeFromBal) {
                    uniV2Router.swapTokensForExactETH(
                        feeAmount,
                        amountStableToRedeem - amountStableForDebt,
                        pathFee,
            ...
            // Borrow more
            uint256 amountVolBorrowed = volPos.owned - volPos.debt;
            code = _tokens.cVol.borrow(amountVolBorrowed);
            require(
                code == 0,
                string(
                    abi.encodePacked(
                        "DNPair: fuse borrow more ",
                        Strings.toString(code)
                    )
                )
            );
            ...
            uint256 amountStableToLend = uniV2Router.swapExactTokensForTokens(
                amountVolBorrowed,
                1,
                pathVolToStable,
                address(this),
        ...
        require(
            volPos.bps >= mb.min && volPos.bps <= mb.max,
            "DNPair: debt not within range"
        );
    }
    ...
    function _getDebtBps(Tokens memory _tokens)
        private
        returns (VolPosition memory volPos)
    {
        volPos.owned =
    ...
    function _mintLiquidity(
        ...
        if (_totalSupply == 0) {
            liquidity =
                Maths.sqrt(amountStable * amountVol) -
                MINIMUM_LIQUIDITY;
