...
contract UniswapLpTokenPriceOracle is PriceOracle {
  using SafeMathUpgradeable for uint256;
    ...
    function _price(address token) internal view virtual returns (uint) {
        ...
        uint token0FairPrice = token0 == weth ? 1e18 : BasePriceOracle(msg.sender).price(token0).mul(1e18).div(10 ** uint256(ERC20Upgradeable(token0).decimals()));
        uint token1FairPrice = token1 == weth ? 1e18 : BasePriceOracle(msg.sender).price(token1).mul(1e18).div(10 ** uint256(ERC20Upgradeable(token1).decimals()));
